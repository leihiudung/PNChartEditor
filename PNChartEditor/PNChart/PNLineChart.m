...
@implementation PNLineChart{
    // 这里的参数,全是改造时添加的
    NSInteger _imageWitdh;
    NSInteger _imageHeight;
    BOOL _showInflexion;
    NSString *_inflexionText;
    NSMutableArray<CATextLayer *> *_labelLayer; // 画线中时间的 textlayer
}
...
- (id)initWithFrame:(CGRect)frame {
    ...
    self.xPosition = frame.size.width;
    self.yPosition = frame.size.height;
    _imageWitdh = 30;
    _imageHeight = 30;
    _labelLayer = [NSMutableArray arrayWithCapacity:9];
    if (self) {
        [self setupDefaultValues];
    }
...
// 设置太阳日出日落时间
- (void)setSunRise:(NSString *)sunRise andSunSet:(NSString *)sunSet {
...
- (void)setYLabels:(NSArray *)yLabels withHeight:(CGFloat)height {
    ...
    if (_showLabel) {
        ...
        for (int index = 0; index < yLabels.count; index++) {
            ...
            PNChartLabel *label = [[PNChartLabel alloc] initWithFrame:CGRectMake(_xPosition - 25, y, (NSInteger) _chartMarginLeft * 0.9, (NSInteger) _yLabelHeight)];
            [label setTextAlignment:NSTextAlignmentRight];
...
- (void)setXLabels:(NSArray *)xLabels withWidth:(CGFloat)width {
    ...
    if (_showLabel) {
        for (int index = 0; index < xLabels.count; index++) {
            ...
            NSInteger x = (index * _xLabelWidth );
...
- (void)strokeChart {
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        // 填充 point 中的颜色
        pointLayer.fillColor = chartData.inflexionPointFillColor;
...
- (void)calculateChartPath:(NSMutableArray *)chartPath andPointsPath:(NSMutableArray *)pointsPath andPathKeyPoints:(NSMutableArray *)pathPoints andPathStartEndPoints:(NSMutableArray *)pointsOfPath {
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        for (NSUInteger i = 0; i < chartData.itemCount; i++) {
            _showInflexion = [chartData.needToShowInflexionPointText[i] boolValue];
            //            if (_showInflexion) {
            _inflexionText = chartData.showInflexionPointText[i];
            //                inflexionIndex++;
            //            }
            ...
            if (chartData.inflexionPointStyle == PNLineChartPointStyleCircle) {
                ...
                //                }
                //                else {
                //                    CGPointMake(circleRect.origin.x + (circleRect.size.width / 2), circleRect.origin.y + (circleRect.size.height / 2));
                //                    [pointPath moveToPoint:CGPointMake(inflexionWidth, circleCenter.y + inflexionWidth / 2)];
                //                }
...
- (void)setChartData:(NSArray *)data {
    if (data != _chartData) {
        ...
        for (PNLineChartData *chartData in data) {
            ...
            // chartLine 默认的线是 whiteColor. 这里修改为 clearColor
...
// 区间竖线
- (void)drawColorInDiffArea:(CGFloat)xAxisWidth {
    
    NSString *sunRiseHourStr = [_sunRise substringToIndex:2];
    NSString *sunRiseMinStr = [_sunRise substringWithRange:NSMakeRange(3, 2)];
    NSString *sunSetHourStr = [NSString stringWithFormat:@"%ld", [_sunSet substringToIndex:2].integerValue + 12];
    NSString *sunSetMinStr = [_sunSet substringWithRange:NSMakeRange(3, 2)];
    
    CGFloat sunRisePosition = [self positionInYAxis:sunRiseHourStr andMin:sunRiseMinStr];
    CGFloat sunSetPosition = [self positionInYAxis:sunSetHourStr andMin:sunSetHourStr];
    ...
    // 日出时间轴
    CGContextMoveToPoint(ctx, sunRisePosition, 0);
    CGContextAddLineToPoint(ctx, sunRisePosition, _yPosition);
    CGContextStrokePath(ctx);
    
    // 日落时间轴
    CGContextMoveToPoint(ctx, sunSetPosition, 0);
    CGContextAddLineToPoint(ctx, sunSetPosition, _yPosition);
    CGContextStrokePath(ctx);
    
    // 这里主要是做 x 轴到顶部之间的距离
    CGPoint point = CGPointMake(2 * _chartMarginLeft + (0 * _xLabelWidth), _chartMarginBottom + _chartCavanHeight);
    //    CGContextSetRGBFillColor(ctx, 1, 1, 1, 0.7);
    CGContextSetRGBFillColor(ctx, 71.0 / 255.0, 81 / 255.0, 90 / 255.0, 0.7);
    CGContextFillRect(ctx, CGRectMake(_chartMarginBottom, 0, sunSetPosition - _chartMarginBottom, point.y));
    
    CGContextSetRGBFillColor(ctx, 91 / 255.0, 89 / 255.0, 70 / 255.0, 0.7);
    CGContextFillRect(ctx, CGRectMake(sunRisePosition, 0, sunSetPosition - sunRisePosition, point.y));
    
    CGContextSetRGBFillColor(ctx, 71.0 / 255.0, 81 / 255.0, 90 / 255.0, 0.7);
    CGContextFillRect(ctx, CGRectMake(sunSetPosition, 0, xAxisWidth - sunSetPosition, point.y));
    
    //    CGContextTranslateCTM(ctx, 0, [UIImage imageNamed:@"sun_rise"].size.height);
    //    CGContextScaleCTM(ctx, 1.0, -1.0);
    UIGraphicsPushContext( ctx );
    UIImage *sunRiseImg = [UIImage imageNamed:@"sun_rise"];
    [sunRiseImg drawInRect:CGRectMake(sunRisePosition - _imageWitdh / 2, _chartMarginBottom + _chartCavanHeight - _imageHeight, sunRiseImg.size.width, sunRiseImg.size.height)];
    UIGraphicsPopContext();
    
    //    CGContextDrawImage(ctx, CGRectMake(sunRisePosition - _imageWitdh / 2, _chartMarginBottom + _chartCavanHeight - _imageHeight, _imageWitdh, _imageHeight), sunRiseImg.CGImage);
    
    
    UIGraphicsPushContext( ctx );
    UIImage *sunSetImg = [UIImage imageNamed:@"sun_set"];
    [sunSetImg drawInRect:CGRectMake(sunSetPosition - _imageWitdh / 2, _chartMarginBottom + _chartCavanHeight - _imageHeight, sunSetImg.size.width, sunSetImg.size.height)];
    UIGraphicsPopContext();
    //    CGContextDrawImage(ctx, CGRectMake(sunSetPosition - _imageWitdh / 2, _chartMarginBottom + _chartCavanHeight - _imageHeight, _imageWitdh, _imageHeight), sunSetImg.CGImage);
}

- (CGFloat)positionInYAxis:(NSString *)hour andMin:(NSString *)min {
    NSInteger hourInt = hour.integerValue;
    CGFloat xPosition = 0.f;
    if (hourInt > 0 && hour <=3) {
        xPosition += _xLabelWidth * 0;
        xPosition += _xLabelWidth / (3 * 60) * ((hourInt - 0) * 60.0 + min.integerValue);
    } else if (hourInt > 3 && hourInt <= 6){
        xPosition += _xLabelWidth * 1;
        xPosition += _xLabelWidth / (3 * 60) * ((hourInt - 3) * 60.0 + min.integerValue);
    } else if (hourInt > 6 && hourInt <= 9){
        xPosition += _xLabelWidth * 2;
        xPosition += _xLabelWidth / (3 * 60) * ((hourInt - 6) * 60.0 + min.integerValue);
    } else if (hourInt > 9 && hourInt <= 12){
        xPosition += _xLabelWidth * 3;
        xPosition += _xLabelWidth / (3 * 60) * ((hourInt - 9) * 60.0 + min.integerValue);
    } else if (hourInt > 12 && hourInt <= 15){
        xPosition += _xLabelWidth * 4;
        xPosition += _xLabelWidth / (3 * 60) * ((hourInt - 12) * 60.0 + min.integerValue);
    } else if (hourInt > 15 && hourInt <= 18){
        xPosition += _xLabelWidth * 5;
        xPosition += _xLabelWidth / (3 * 60) * ((hourInt - 15) * 60.0 + min.integerValue);
    } else if (hourInt > 18 && hourInt <= 21){
        xPosition += _xLabelWidth * 6;
        xPosition += _xLabelWidth / (3 * 60) * ((hourInt - 18) * 60.0 + min.integerValue);
    } else if (hourInt > 21 && hourInt < 24){
        xPosition += _xLabelWidth * 7;
        xPosition += _xLabelWidth / (3 * 60) * ((hourInt - 21) * 60.0 + min.integerValue);
    }
    
    //    xPosition += _xLabelWidth / 60 * min.integerValue;
    return xPosition + 10;
}
...
- (void)drawRect:(CGRect)rect {
    if (self.isShowCoordinateAxis) {
        ...
        CGContextMoveToPoint(ctx, _xPosition - 35.0 + yAxisOffset, 0);
        CGContextAddLineToPoint(ctx, _xPosition - 35.0 + yAxisOffset, yAxisHeight);
        CGContextAddLineToPoint(ctx, 20, yAxisHeight);
        CGContextStrokePath(ctx);
        ...
        CGContextMoveToPoint(ctx, _xPosition - 35.0 + yAxisOffset - 3, 6);
        CGContextAddLineToPoint(ctx, _xPosition - 35.0 + yAxisOffset, 0);
        CGContextAddLineToPoint(ctx, _xPosition - 35.0 + yAxisOffset + 3, 6);
        CGContextStrokePath(ctx);
        ...
        CGContextMoveToPoint(ctx, 20 + 6, yAxisHeight - 3);
        CGContextAddLineToPoint(ctx, 20, yAxisHeight);
        CGContextAddLineToPoint(ctx, 20 + 6, yAxisHeight + 3);
        CGContextStrokePath(ctx);
        
        //        CGContextDrawImage(ctx, CGRectMake(20 + 6÷, yAxisHeight, 30, 30), [UIImage imageNamed:@"IconRocket"].CGImage);
        
        [self drawColorInDiffArea:xAxisWidth];
...
- (CATextLayer *)createPointLabelFor:(CGFloat)grade pointCenter:(CGPoint)pointCenter width:(CGFloat)width withChartData:(PNLineChartData *)chartData {
    NSArray *tempLayerArray = _labelLayer.copy;
    // 这里还有代码有冗余
    if ( _labelLayer.count >= 9) {
        for (int i = 0; i < _labelLayer.count; i++) {
            CATextLayer *textLayer = _labelLayer[i];
            [textLayer removeFromSuperlayer];
        }
        [_labelLayer removeAllObjects];
    }
    ...
    [_labelLayer addObject:textLayer];
    
    if (chartData.pointLabelFormat != nil) {
        //        if (_showInflexion) {
        
        [textLayer setString:_inflexionText];
        //        } else {
        //            [textLayer setString:@""];
        //        }
    } else {
